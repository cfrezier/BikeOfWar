    ...
    while (_g1 < _g) {
        ...
        if (truck.owner.id == this.id && truck.currentStation != null) {
            ...
            if(truck.next === "load"){
                ...
                if (truck.currentStation.bikeNum < 4 && truck.bikeNum > 0) {
                    var unload = 0;
                    ...
                    if (truck.bikeNum < Game.TRUCK_NUM_SLOT && truck.currentStation.bikeNum > 8) {
                        // On charge si on peut (on en laisse 5)
                        var load = (truck.currentStation.bikeNum - 5);
            ...
            if(truck.next === "move") {
                // Puis on bouge
                var distance = 0, destId = null, destDistance = 999999999999999;
                for (var j = 0; j < context.stations.length; j++) {

                    if(
                        (truck.bikeNum <= 2 && context.stations[j].bikeNum > 8)
                        ||
                        (truck.bikeNum > 2 && context.stations[j].bikeNum < 4 )
                        ) {
                        distance = GameUtils.getTravelDuration(truck, context.stations[j]);
                        if (destDistance > distance && distance > 0) {
                            destId = context.stations[j].id;
                            destDistance = distance;
                        }
                    }
                }
                if (destId != null && truckDests.indexOf(destId) === -1) {
