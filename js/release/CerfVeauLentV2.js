...
var getOrders = function (context) {
    ...
    while (_g1 < _g) {
        ...
        if (truck.owner.id == this.id && truck.currentStation != null) {
            ...
            if(truck.next === "load"){
                // Si la station manque de vélos, on décharge tout
                if (truck.currentStation.bikeNum < truck.currentStation.slotNum / 4 + 1 && truck.bikeNum > 0) {
                    var unload = 0;
                    if(truck.bikeNum > 5) { unload = 5;} else {unload = truck.bikeNum;}

                    result.push(new UnLoadingOrder(truck.id, truck.currentStation.id, unload));
                    truck.next = "willMove";
                } else {
                    if (truck.bikeNum < Game.TRUCK_NUM_SLOT && truck.currentStation.bikeNum > truck.currentStation.slotNum / 2) {
                        // On charge si on peut (on en laisse 5)
                        var load = Math.floor(truck.currentStation.bikeNum - truck.currentStation.slotNum / 4 + 3);
                        if(truck.bikeNum + load > Game.TRUCK_NUM_SLOT) {
                            load = Game.TRUCK_NUM_SLOT - truck.bikeNum;
                        }
                        if(load > truck.currentStation.bikeNum) { load = 1;}
                        if(load <= 0) { load = 2; }

                        result.push(new LoadingOrder(truck.id, truck.currentStation.id, load));
                        truck.next = "willMove";
                    } else {
                        if(truck.currentStation.owner !== undefined && truck.currentStation.owner.id !== this.id) {
                            // On va quand même piquer la station avant de repartir !
                            debugMessage += "Je prends avant de repartir...";
                            if(truck.bikeNum > 1) {
                                result.push(new UnLoadingOrder(truck.id, truck.currentStation.id, 1));
                                truck.next = "willMove";
                            } else {
                                if(truck.currentStation.bikeNum > 1) {
                ...
                truck.previousStation = truck.currentStation.id;
            }

            if(truck.next === "move") {
                // Puis on bouge
                var destId = null, destScore = 0, score, trend;
                for (var j = 0; j < context.stations.length; j++) {

                    var processedStation = context.stations[j];

                    // Importance de la distance proche sur 250
                    score = (10 - GameUtils.getTravelDuration(truck, processedStation)) * 30;

                    if(processedStation.owner = this.id) {
                        // Mes stations qui font perdre des points (inférieur à 1/4 ou > 3/4 occupation) sur 150
                        if(processedStation.bikeNum <= processedStation.slotNum / 4 && truck.bikeNum > 4) {
                            score += 3000;
                        }

                        if(processedStation.bikeNum >= processedStation.slotNum / 4 * 3 && truck.bikeNum < 6) {
                            score += 3000;
                        }
                    } else {
                        if(processedStation.owner !== undefined) {
                            // Stations adverses
                            if(processedStation.bikeNum <= processedStation.slotNum / 4 && truck.bikeNum > 4) {
                                score -= 6000;
                            }
                    ...
                    // Si je suis vide, je préfère les stations remplies
                    score += (truck.bikeNum < 3 ? 1 : 0) * processedStation.bikeNum * 45;

                    // Si je suis plein, je prfère les stations vides
                    score += (truck.bikeNum > 6 ? 1 : 0) * (25 - processedStation.bikeNum) * 45;

                    if(truck.previousStation === processedStation.id) {
                        score = 0;
                    }

                    if (score > destScore) {
                        destScore = score;
                        destId = processedStation.id;
                    }
                }

                if (destId != null && destination[(i+1)%2] !== destId) {
                    destination[i] = destId;
                    result.push(new MoveOrder(truck.id, destId));
                    truck.next = "load";
                    debugMessage += "destScore : " + destScore + "\n";
                }

            }

            if(truck.next === "willMove") {
                truck.next = "move";
            }
        }
    }
